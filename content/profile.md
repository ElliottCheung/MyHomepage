---
# title: "Curriculum Vitae"
description: "Profile"
hidemeta: true
layout: "profile"
---

<div align="center" style="font-size: 32px; font-weight: bold;">
    Curriculum Vitae
</div>

## {{<icon "education" 19>}} &nbsp; Education

{{<profile_block title="Tsinghua University" location="Beijing, China" date="2021-Present" sub="Ph.D. in Management Science and Engineering (Information Systems)">}}
+ Advisor: <a style="color:  var(--linkblue);" href="https://www.sem.tsinghua.edu.cn/info/1210/31859.htm"> Zhijie Lin</a>
+ GPA: 3.94/4.00
{{</profile_block>}}

{{<profile_block title="Hong Kong University of Science and Technology" date="2025-2026" location="Hong Kong SAR, China" sub="PG Visiting Student in Information Systems">}}
+ Advisor: <a style="color: var(--linkblue);" href="https://isom.hkust.edu.hk/faculty-and-staff/directory/jwang">Jing Wang</a>
{{</profile_block>}}

{{<profile_block title="Nankai University" date="2018-2021" location="Tianjin, China" sub="B.Sc. in Finance">}}
{{</profile_block>}}

{{<profile_block title="Tianjin Unviersity" date="2017-2021" location="Tianjin, China"  sub="B.B.A. in Electronic Commerce">}}
+ Advisor: <a style="color:  var(--linkblue);" href="https://come.tju.edu.cn/info/1089/3755.htm"> Yi Wu</a>
+ GPA: 3.92/4.00 (rank 1st for 4 consecutive years)
{{</profile_block>}}


<br>


## {{<icon "file" 19>}}  &nbsp; Publication and Working Paper

#### Journal Publication

{{<profile_paper_block 
    title="AI Orchestrator: How Recommendation Algorithms Shape Complementor Strategy and Market Equality"
    status="published"
    year="2026"
    source="Strategic Management Journal"
    level="UTD-24;FT-50;AJG 4*"
    author="<span class='special-author'>Xiaowei (Elliott) Zhang</span>, Siliang (Jack) Tong, Xueming Luo, Zhijie Lin, Jing (Aaron) Li"
    >}}
{{</profile_paper_block>}}

#### Working Paper

{{<profile_paper_block 
    title="Item-level Feedback and Overall Rating: Evidence from a Temporary Granularity Penalty"
    status="working paper"
    year="2026"
    source="Work in Progress"
    level=""
    author="<span class='special-author'>Xiaowei (Elliott) Zhang</span>, Jing Wang, Zhijie Lin"
    >}}
{{</profile_paper_block>}}

{{<profile_paper_block 
    title="Follow Your Heart: The Role of Interest in the Sharing Economy"
    status="working paper"
    year="2025"
    source="Working Paper"
    level=""
    author="<span class='special-author'>Xiaowei (Elliott) Zhang</span>, Zhijie Lin, Jing Wang, Jason Chan"
    >}}
{{</profile_paper_block>}}


#### Journal Publication (in Chinese)

{{<profile_paper_block 
    title="Hosts′ Motivations to Forgo Screening Their Guests on Sharing Accommodation Platforms: Based on the Intra-industry Competition"
    status="published"
    year="2022"
    source="Journal of Management Science (管理科学)"
    volume="35"
    issue="1"
    pages="3-16"
    level="FMS-T1;NFSC-A"
    author="Yi Wu, <span class='special-author'>Xiaowei Zhang</span>, Zhijie Lin"
    >}}
{{</profile_paper_block>}}


#### Conference Paper

{{<profile_paper_block 
    title="Follow Your Heart: The Role of Interest in the Sharing Economy"
    location="Bangkok <span class='flag'>🇹🇭</span>"
    source="45th Internation Conference on Information Systems (ICIS 2024)"
    level=""
    author="<span class='special-author'>Xiaowei (Elliott) Zhang</span>, Zhijie Lin, Jing Wang, Jason Chan"
    >}}
{{</profile_paper_block>}}

{{<profile_paper_block 
    title="Helmsman of the Masses? Evaluate the Opinion Leadership of Large Language Models in the Werewolf Game"
    location="Philadelphia <span class='flag'>🇺🇸</span>"
    source="First Conference on Language Modeling (COLM 2024)"
    level=""
    author="Silin Du, <span class='special-author'>Xiaowei Zhang</span>"
    >}}
{{</profile_paper_block>}}

{{<profile_paper_block 
    title="All that Glitters is not Gold: Understanding the Impacts of Platform Recommendation Algorithm Changes on Complementors in the Sharing Economy"
    location="Hyderabad <span class='flag'>🇮🇳</span>"
    source="44th International Conference on Information Systems (ICIS 2023)"
    level=""
    author="Siliang (Jack) Tong, <span class='special-author'>Xiaowei (Elliott) Zhang</span>, Xueming Luo, Zhijie Lin, Jing (Aaron) Li"
    >}}
{{</profile_paper_block>}}


{{<profile_paper_block 
    title="A Good Neighbor, A Found Treasure? Examining the Spatial Determinants on Airbnb Listings Prices"
    location="Hangzhou <span class='flag'>🇨🇳</span>"
    source="Third POMS International Conference in China (POMS-CN 2023)"
    level=""
    author="<span class='special-author'>Xiaowei (Elliott) Zhang</span>, Zhijie Lin"
    >}}
{{</profile_paper_block>}}

{{<profile_paper_block 
    title="No Change, No Growth? The Role of Competition in the Effectiveness of Airbnb Hosts’ Dynamic Strategies"
    location="Ningbo <span class='flag'>🇨🇳</span>"
    source="15th China Summer Workshop on Information Management (CSWIM 2022)"
    level=""
    author="<span class='special-author'>Xiaowei Zhang</span>, Zhijie Lin"
    >}}
{{</profile_paper_block>}}




<br>

## {{<icon "award" 19>}}  &nbsp; Selected Honors and Awards

{{<award_grid>}}
    {{<award_block title="Weimin Zheng Scholarship" source="Tsinghua University" date="2026" >}}
    {{<award_block title="Greenberg Academic Fellowship" source="Tsinghua University" date="2025" >}}
    {{<award_block title="National Scholarship (国家奖学金) (Doctoral)" source="Ministry of Education, China" date="2025" >}}
    {{<award_block title="Comprehensive Scholarship" source="Tsinghua University" date="2024, 2023" >}}
    {{<award_block title="Outstanding Teaching Assistant Award" source="Tsinghua University" date="2022" >}}
    {{<award_block title="Outstanding Graduate" source="Tianjin University" date="2021" >}}
    {{<award_block title="Outstanding Undergraduate Thesis" source="Tianjin University" date="2021" >}}
    {{<award_block title="National Scholarship (国家奖学金) (Undergraduate)" source="Ministry of Education, China" date="2020" >}}
    {{<award_block title="Student Science and Technology Talent Award" source="Tianjin University" date="2020" >}}
    {{<award_block title="Outstanding Undergraduate Student Award" source="Tianjin University" date="2019" >}}
    {{<award_block title="Outstanding Student Scholarship" source="Tianjin University" date="2019" >}}
    {{<award_block title="NITORI International Scholarship" source="NITORI International Scholarship Foundation" date="2018" >}}
{{</award_grid>}}
